...
def readIntensity(dataDir, photoName, plotName, lamp, surface):
   ...
   monoCut = np.flip(monoPhoto[425:1025, 1130:1290].swapaxes(0, 1), axis = 1 )# (транспонирование) изображения
   intensity = np.mean(monoCut, axis=0) # усреднение яркости

   photo = imageio.imread(photoName)
   cut = np.flip(photo[425:1025, 1130:1290, 0:3].swapaxes(0,1), axis = 1)
...
def findColorMax(color, photoName):
   photo = imageio.imread(photoName)
   if color == "green":
      cut = np.flip(photo[425:1025, 1130:1290, 1:2].swapaxes(0,1), axis = 1)
   if color == "blue":
      cut = np.flip(photo[425:1025, 1130:1290, 2:3].swapaxes(0,1), axis = 1)
